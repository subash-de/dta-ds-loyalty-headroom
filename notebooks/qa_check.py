...
offer_desc_short = {14055: "£5 off spend £30",
...
display(export_offers
        .groupby("offer_id", "desc", "desc_short", "upper_lim")
        .count()
        .orderBy("upper_lim")
       )
...
display(export_offers.groupby("account_id")
        .agg(F.count("*").alias("account_counts"),
...
# DBTITLE 1,VIP List
vip = spark.read.csv("dbfs:/mnt/centralds/offerallocation/TMO/vip_customers/vip_list_20210928.csv", header=True, inferSchema=True)

export_vip = (export_offers
              .join(vip, on="account_id")
             )
...
display(export_offers_tco
        .groupby("offer_id", "desc_short", "upper_lim", "TCO")
        .count()
...
display(export_offers_tco
        .groupby("offer_id", "upper_lim", "desc_short", "TCO")
        .count()
        .orderBy("upper_lim", "TCO")
       )
