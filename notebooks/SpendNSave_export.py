...
dbutils.widgets.text("sparks_account_table","sparks_account","") #analytics_trans_prod.sparks_account
dbutils.widgets.text("customer_group_path","dbfs:/mnt/centralds/offerallocation/FPO/prod_runs/220208/prod_input/tco_customer_groups_v1/customer_group=2","")# dbfs:/mnt/centralds/offerallocation/FPO/prod_runs/220207/prod_input/tco_customer_groups_v2/customer_group=2
dbutils.widgets.text("Sns_out_path","dbfs:/mnt/centralds/offerallocation/SNS/prod_runs/2208015/export/SpendNSave220815.csv","")# dbfs:/mnt/centralds/offerallocation/SNS/prod_runs/220801/export/SpendNSave220801.csv
dbutils.widgets.text("cds_wheel_path","dbfs:/FileStore/jars/offer_engine/dev/20220808.01/cdsutils-0.0.8.2021061002-py3-none-any.whl","")
dbutils.widgets.text("oe_wheel_path","dbfs:/FileStore/jars/offer_engine/dev/20220808.01/offerallocation-20220801.1-py3-none-any.whl","")
...
offer_details=spark.sql(f"""select DISTINCT(offer_id),desc from {sns_headroom_schema+"."+sns_headroom_table}""")
...
offer_details_new = (
  ...
  .withColumn(
    "offer_value",
    get_offer_value_udf(F.col("desc"))
  )
)
...
sns_current=spark.sql(f"""select p.account_id,c.offer_id from {sparks_account_schema+"."+sparks_account_table} as p INNER JOIN {sns_headroom_schema+"."+sns_headroom_table} as c ON p.cust_id=c.cust_id""")
...
logger.info(f""" Total Unique Customer Count:{sns_pre_final.select('account_id').count()}""")

# COMMAND ----------

sns_final=(
  sns_pre_final
  .join(offer_details_new.select("offer_id","offer_value"),how="left",on="offer_id")
