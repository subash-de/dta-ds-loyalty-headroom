...
trans_before_june_accu = persist_utils.read_table(
    table_name = 'loyalty_azlab_prod.legacy_all_transaction_line',
    where= f"{config_sim['date_col']} >={start_date}"
           and f"{config_sim['date_col']} <= {config.dates.rolling_sum_etl_date}"
           and f"sparks_reg_date <= {start_date}"
           and "trans_line_type = 'S'"
           and f"division_id = {config_sim['department']}"
...
weekly_sales = simulation_utils.calculate_weekly_rolling_sum(trans_before_june_accu, 
...
percentile_df.display()

# COMMAND ----------

final_df = baseline_per_customer.join(percentile_df.select(['cust_id']+percentile_columns), on=config_sim['grouping_cols'], how="left")
...
customer_category_etl_date = int(config.dates.rolling_sum_etl_date.replace("-",""))

# COMMAND ----------

customer_category_etl_date
...
customer_df = spark.sql(f"""
                  ...
                  where yyyymmdd <= {customer_category_etl_date})
                  WHERE row_num = 1
                  """)
