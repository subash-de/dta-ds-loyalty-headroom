...
recapture = spark.sql("select * from ciu_azlab_dev.20211221_ns_recapture_campaign_agg_final_offer where fd_offer_eligible='Y'")
...
allocation_path = "dbfs:/mnt/centralds/offerallocation/headroom/prod_run/export/202112/export_v3"
...
customers_path = "dbfs:/mnt/centralds/offerallocation/headroom/prod_run/prod_input/202112/customers"
...
pred = (spark.read.parquet(pred_path)
        ...
        .join(recapture.select(F.col("sparks_account_id").alias("account_id")).distinct(), on="account_id", how="leftanti")
       )
...
hhs = pred.select("experian_hh_composition").distinct().rdd.map(lambda x: x[0]).collect()
...
pred_scores = (pred
               .withColumn("pct_error", 100.*(F.col("prediction_out")-F.col("total_spend"))/(F.col("total_spend")))
               .groupby("account_id", "experian_hh_composition", "segmentation")
               .agg(F.sum("total_spend").alias("sum_total_spend"),
...
display(pred_scores.filter((F.col("mean_pct_error")>=0) & (F.col("mean_pct_error")<=100)))
...
pred_scores.filter((F.col("mean_pct_error")>=0) & (F.col("mean_pct_error")<=100)).count()

# COMMAND ----------

group1 = (pred_scores
#           .filter((F.col("mean_pct_error")>=5) & (F.col("mean_pct_error")<=20))
          .withColumn("outlier", F.when((F.col("mean_pct_error")>=5) & (F.col("mean_pct_error")<=20), 0).otherwise(1))
         )
...
offer_desc = {14055: "£5 off when you spend £30 on M&S food in store",
              13992: "£5 off when you spend £40 on M&S food in store",
              13993: "£5 off when you spend £50 on M&S food in store",
              13994: "£7 off when you spend £70 on M&S food in store",
              13995: "£9 off when you spend £90 on M&S food in store",
...
group1_allocation = (group1_allocation
                     .withColumn("used_headroom_frac", F.when(F.col("mean_pct_error")>=max_increase, (1.+max_increase/100.))
                                 .when(F.col("mean_pct_error")<=min_increase, (1.+min_increase/100.))
                                 .otherwise(1.+F.col("mean_pct_error")/100.)
                                )
                     .withColumn("total_used_headroom", F.col("sum_total_spend_month")*F.col("used_headroom_frac"))
...
group1_allocation_final = (group1_allocation_temp
                           # If very large headroom. Probably some outliers. For now random spread these offers over the top offer range.
                           .withColumn("offer_id", F.when((F.col("total_used_headroom")>=large_lim), get_large_offer(F.col("rand")))
                                       .otherwise(F.col("offer_id")))
                           # If offer Id is still null then an outlier. Give a random small offer.
                           .withColumn("offer_id", F.when((F.col("offer_id").isNull()), get_small_offer(F.col("rand")))
                                       .otherwise(F.col("offer_id")))
...
display(group1_allocation_final
        .groupby("offer_id", "desc", "upper_lim")
        .count()
        .orderBy("upper_lim")
       )
...
# add flag 
custs = spark.read.parquet("dbfs:/mnt/centralds/offerallocation/headroom/prod_run/prod_input/202112/customers_v3")
custs_recent = custs.filter(F.col("recent_purchase")==1).select("account_id").distinct()


group1_allocation_final_out = (group1_allocation_final
                               .join(custs_recent.withColumn("analysis_flag", F.lit(1)), on="account_id", how="left")
                               .fillna(0, subset=["analysis_flag"])
                               .select(out_columns)
                              )

out_columns = ["account_id", "offer_id", F.lit(2).alias("test_group"), "analysis_flag"]
group1_allocation_final_out.coalesce(1).write.csv(allocation_path, header=True, mode="overwrite")
