...
class SegmentationDataManager(BaseManager):
    ...
    def get(
        ...
        """
        # Get transactions in timeframe [lookback_date, etl_date]
        spent_in_period = self._find_spend_in_period(
            trx_line, sparks_custs, customer_input
        )

        sparks_custs_spent_in_period = self._sparks_customer_mosaic_data(
            spent_in_period, cust_master
        )
    ...
    def _find_spend_in_period(
        ...
        trx_line_between_date = (
            trx_line_with_date.filter(self.get_common_filters())
        ...
        if customer_input is not None:
            trx_line_between_date = trx_line_between_date.join(
                customer_input.select(self.user_id).distinct(),
                on=self.user_id,
                how="inner",
            )

        spent_in_period = (
            trx_line_between_date.groupby(self.user_id)
            .pivot("l1_id", l1_ids)
            .agg(
            ...
            .fillna(
                ...
                subset=[
                    f"{l1}_{val}"
                    for val in ["total_spend", "total_items", "total_baskets"]
                    for l1 in l1_ids
                ],
            )
            .withColumn(
                "total_spend", F.col("FD_total_spend") + F.col("GM_total_spend")
            )
            .withColumn(
                "total_items", F.col("FD_total_items") + F.col("GM_total_items")
            )
            .withColumn(
                "total_baskets", F.col("FD_total_baskets") + F.col("GM_total_baskets")
            )
            ...
            .withColumn(
                "FD_baskets", F.col("FD_total_baskets") / F.col("total_baskets")
            )
            .withColumn(
                "GM_baskets", F.col("GM_total_baskets") / F.col("total_baskets")
            )
    ...
    def _sparks_customer_mosaic_data(
        self, spent_in_period: DataFrame, cust_master: DataFrame
    ) -> DataFrame:
        """
        Join on to sparks customer transaction DataFrame the experian mosaic features from the cust_master
        factory table.
        """
        sparks_custs_spent_in_period = (
            spent_in_period
    ...
    def clean_demog_data(self, sparks_custs_spent_in_period: DataFrame):
        """ """
        cleaned_demog_data = (
            sparks_custs_spent_in_period
            ...
            .withColumn(
                "age",
                F.when(F.col("age").between(18, 120), F.col("age")).otherwise(None),
            )
            .withColumn(
                "experian_affluence",
            ...
            .withColumn(
                "age_to_use",
                F.coalesce(F.col("age"), F.col("experian_age_n"), F.lit(56)),
            )
            .withColumn(
                "banded_age",
                F.when(F.col("age_to_use").between(18, 25), "18-25")
    ...
    def _prepare_dataset(self, df: DataFrame, prefix: str = "Cat_") -> DataFrame:
        """
        ...
        df = df.select(
            ...
            F.concat(
                F.lit(prefix), F.coalesce(F.col("experian_hh_composition"), F.lit("U"))
            ).alias("experian_hh_composition"),
            F.concat(
                F.lit(prefix), F.coalesce(F.col("experian_mosaic_uk_type"), F.lit("U"))
            ).alias("experian_mosaic_uk_type"),
            F.when(F.col("gender") == "F", 1).otherwise(0).alias("gender_F"),
            F.when(F.col("gender") == "M", 1).otherwise(0).alias("gender_M"),
            F.when(F.col("experian_hh_composition") == f"{prefix}00", 1)
            .otherwise(0)
            .alias("hh_comp_00"),
            F.when(F.col("experian_hh_composition") == f"{prefix}01", 1)
            .otherwise(0)
            .alias("hh_comp_01"),
            F.when(F.col("experian_hh_composition") == f"{prefix}02", 1)
            .otherwise(0)
            .alias("hh_comp_02"),
            F.when(F.col("experian_hh_composition") == f"{prefix}03", 1)
            .otherwise(0)
            .alias("hh_comp_03"),
            F.when(F.col("experian_hh_composition") == f"{prefix}04", 1)
            .otherwise(0)
            .alias("hh_comp_04"),
            F.when(F.col("experian_hh_composition") == f"{prefix}05", 1)
            .otherwise(0)
            .alias("hh_comp_05"),
            F.when(F.col("experian_hh_composition") == f"{prefix}06", 1)
            .otherwise(0)
            .alias("hh_comp_06"),
            F.when(F.col("experian_hh_composition") == f"{prefix}07", 1)
            .otherwise(0)
            .alias("hh_comp_07"),
            F.when(F.col("experian_hh_composition") == f"{prefix}08", 1)
            .otherwise(0)
            .alias("hh_comp_08"),
            F.when(F.col("experian_hh_composition") == f"{prefix}09", 1)
            .otherwise(0)
            .alias("hh_comp_09"),
            F.when(F.col("experian_hh_composition") == f"{prefix}10", 1)
            .otherwise(0)
            .alias("hh_comp_10"),
            F.when(F.col("experian_hh_composition") == f"{prefix}12", 1)
            .otherwise(0)
            .alias("hh_comp_11"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}00", 1)
            .otherwise(0)
            .alias("mosaic_00"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}01", 1)
            .otherwise(0)
            .alias("mosaic_01"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}02", 1)
            .otherwise(0)
            .alias("mosaic_02"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}03", 1)
            .otherwise(0)
            .alias("mosaic_03"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}04", 1)
            .otherwise(0)
            .alias("mosaic_04"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}05", 1)
            .otherwise(0)
            .alias("mosaic_05"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}06", 1)
            .otherwise(0)
            .alias("mosaic_06"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}07", 1)
            .otherwise(0)
            .alias("mosaic_07"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}08", 1)
            .otherwise(0)
            .alias("mosaic_08"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}09", 1)
            .otherwise(0)
            .alias("mosaic_09"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}10", 1)
            .otherwise(0)
            .alias("mosaic_10"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}11", 1)
            .otherwise(0)
            .alias("mosaic_11"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}12", 1)
            .otherwise(0)
            .alias("mosaic_12"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}13", 1)
            .otherwise(0)
            .alias("mosaic_13"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}14", 1)
            .otherwise(0)
            .alias("mosaic_14"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}15", 1)
            .otherwise(0)
            .alias("mosaic_15"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}16", 1)
            .otherwise(0)
            .alias("mosaic_16"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}17", 1)
            .otherwise(0)
            .alias("mosaic_17"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}18", 1)
            .otherwise(0)
            .alias("mosaic_18"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}19", 1)
            .otherwise(0)
            .alias("mosaic_19"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}20", 1)
            .otherwise(0)
            .alias("mosaic_20"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}21", 1)
            .otherwise(0)
            .alias("mosaic_21"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}22", 1)
            .otherwise(0)
            .alias("mosaic_22"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}23", 1)
            .otherwise(0)
            .alias("mosaic_23"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}24", 1)
            .otherwise(0)
            .alias("mosaic_24"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}25", 1)
            .otherwise(0)
            .alias("mosaic_25"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}26", 1)
            .otherwise(0)
            .alias("mosaic_26"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}27", 1)
            .otherwise(0)
            .alias("mosaic_27"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}28", 1)
            .otherwise(0)
            .alias("mosaic_28"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}29", 1)
            .otherwise(0)
            .alias("mosaic_29"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}30", 1)
            .otherwise(0)
            .alias("mosaic_30"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}31", 1)
            .otherwise(0)
            .alias("mosaic_31"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}32", 1)
            .otherwise(0)
            .alias("mosaic_32"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}33", 1)
            .otherwise(0)
            .alias("mosaic_33"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}34", 1)
            .otherwise(0)
            .alias("mosaic_34"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}35", 1)
            .otherwise(0)
            .alias("mosaic_35"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}36", 1)
            .otherwise(0)
            .alias("mosaic_36"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}37", 1)
            .otherwise(0)
            .alias("mosaic_37"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}38", 1)
            .otherwise(0)
            .alias("mosaic_38"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}39", 1)
            .otherwise(0)
            .alias("mosaic_39"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}40", 1)
            .otherwise(0)
            .alias("mosaic_40"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}41", 1)
            .otherwise(0)
            .alias("mosaic_41"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}42", 1)
            .otherwise(0)
            .alias("mosaic_42"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}43", 1)
            .otherwise(0)
            .alias("mosaic_43"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}44", 1)
            .otherwise(0)
            .alias("mosaic_44"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}45", 1)
            .otherwise(0)
            .alias("mosaic_45"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}46", 1)
            .otherwise(0)
            .alias("mosaic_46"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}47", 1)
            .otherwise(0)
            .alias("mosaic_47"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}48", 1)
            .otherwise(0)
            .alias("mosaic_48"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}49", 1)
            .otherwise(0)
            .alias("mosaic_49"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}50", 1)
            .otherwise(0)
            .alias("mosaic_50"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}51", 1)
            .otherwise(0)
            .alias("mosaic_51"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}52", 1)
            .otherwise(0)
            .alias("mosaic_52"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}53", 1)
            .otherwise(0)
            .alias("mosaic_53"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}54", 1)
            .otherwise(0)
            .alias("mosaic_54"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}55", 1)
            .otherwise(0)
            .alias("mosaic_55"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}56", 1)
            .otherwise(0)
            .alias("mosaic_56"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}57", 1)
            .otherwise(0)
            .alias("mosaic_57"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}58", 1)
            .otherwise(0)
            .alias("mosaic_58"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}59", 1)
            .otherwise(0)
            .alias("mosaic_59"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}60", 1)
            .otherwise(0)
            .alias("mosaic_60"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}61", 1)
            .otherwise(0)
            .alias("mosaic_61"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}62", 1)
            .otherwise(0)
            .alias("mosaic_62"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}63", 1)
            .otherwise(0)
            .alias("mosaic_63"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}64", 1)
            .otherwise(0)
            .alias("mosaic_64"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}65", 1)
            .otherwise(0)
            .alias("mosaic_65"),
            F.when(F.col("experian_mosaic_uk_type") == f"{prefix}66", 1)
            .otherwise(0)
            .alias("mosaic_66"),
            "FD_spend",
            "GM_spend",
...
class SegmentationManager(BaseManager):
    ...
    def get(self, data: DataFrame, etl_data: Optional[DataFrame] = None) -> DataFrame:
        """
    ...
    def _get_category_filters(self, demog_groups: List[str]) -> DataFrame:
        """
        method to collect category filters used for segmentations.
        """
        demog_cat_filters_str = [
            f"(F.col('{dc}') == '{dc_group}')"
            for (dc, dc_group) in zip(self.cat_cols, demog_groups)
        ]
        demog_cat_filters = eval(" & ".join(demog_cat_filters_str))
        return demog_cat_filters

    def _find_best_KMeans(self, data: DataFrame) -> Dict[str, DataFrame]:
        """
        ...
        for demog_groups in self.category_list:
            ...
            if self.pca_k:
                ...
                pca = sparkPCA(
                    k=self.pca_k, inputCol="standardized", outputCol=output_col
                )
            ...
            if data_count > self.data_lower_lim:
                ...
                evaluator = ClusteringEvaluator(
                    predictionCol="segmentation",
                    featuresCol=output_col,
                ...
                for k in range(self.k_search_min, self.k_search_max):
                    ...
                    fail_tab = (
                        ...
                        .withColumn(
                            "count_fail",
                            F.when(F.col("count") < count_lim, 1).otherwise(0),
                        )
                        .withColumn(
                            "fraction_fail",
                            F.when(F.col("fraction") < self.frac_lim, 1).otherwise(0),
                        )
                        .withColumn(
                            "fail",
                            F.greatest(F.col("count_fail"), F.col("fraction_fail")),
                        )
                    )
                    fail_check = (
                        fail_tab.select(F.max("fail")).rdd.map(lambda x: x[0]).first()
                    )

                    if fail_check == 1:
                        fail_count += 1
                        self.log(
                            f"""{demog_groups_str} - {k}:
                        ...
                        self.log(
                            f"{demog_groups_str} - {k}: \t Silhouette Score: {score}"
                        )

                all_silhouette_score[demog_groups_str] = silhouette_score
                max_silhouette_score_key = max(
                    silhouette_score, key=silhouette_score.get
                )
    ...
    def _join_all_best_KMeans(self, kmeans_dict: Dict[str, DataFrame]) -> DataFrame:
        """
        ...
        for p in range(self.pca_k):
            ...
            final_output = final_output.withColumn(
                f"pca_{p}", extract_element("pca_features")
            )

        return final_output

    def _get_demog_cats(self, data: DataFrame) -> DataFrame:
        """
        This method gets the [self.cat_cols] demographic data from the input dataset.
        """
        category_list = (
            (data.select(self.cat_cols).distinct().orderBy(self.cat_cols))
            .rdd.map(lambda x: [xi for xi in x])
            .collect()
        )
    ...
    def evaluate_cluster_plot(
        ...
        for i, demog_groups in enumerate(self.category_list):
            demog_groups_str = str(demog_groups)
            demog_cat_filters = self._get_category_filters(demog_groups)
            df_temp = df.filter(demog_cat_filters).select(
                [f"pca_{k}" for k in range(pca_k)] + [seg_col]
            )
            if sample:
            ...
            legend1 = ax.legend(
                *scatter.legend_elements(num=predictions),
                loc="upper right",
                title="Classes",
            )
            ...
            cluster_out_png_path = (
                f"{out_root_path}/{out_filename}_{demog_groups_str}.png"
            )
