...
dbutils.fs.ls(
    "dbfs:/mnt/centralds/offerallocation/headroom/development/v0.2/data/202112/v0.0.0/"
)
...
for i, hh in enumerate(hhs):
    df = (
        seg.filter(F.col("experian_hh_composition") == hh)
        .join(sparks_tco, on="cust_id", how="left")
        .join(
            data.select("cust_id", "total_spend_amount").distinct(),
            on="cust_id",
            how="left",
        )
    )
    ...
    legend1 = ax.legend(
        *scatter.legend_elements(num=segmentations),
        loc="upper right",
        title="Segmentation",
    )
    ...
    for s in segmentations:
        ...
        df_[df_.segmentation == s].TCO.value_counts().sort_index().plot(
            kind="bar", ax=axes[5, s]
        )
        axes[5, s].set_title(f"TCO. Segment: {s}")
        df_[df_.segmentation == s].gender.value_counts().sort_index().plot(
            kind="pie", ax=axes[6, s]
        )
