...
# MAGIC %md # Average weekly max basket approach 
# MAGIC This notebook uses the 85th percentile of weekly max basket approach to do recomendation and headroom 
...
prediction_time_range = [20221023, 20230515]
...
display(segmentations_tbl)

# COMMAND ----------

segmentations_tbl.filter(F.col("cust_id") == "6872732896086312431").display()
...
# t1 = spark.sql("select * from loyalty_azlab_prod.headroom_etl_data_p_tbl where campaign = 20230421")
# t1.select("campaign").distinct().display()

# COMMAND ----------

# %sql select * from loyalty_azlab_prod.headroom_etl_data_p_tbl where campaign = 20230421
...
# %sql select * from loyalty_azlab_prod.headroom_etl_data_p_tbl where campaign = 20230424
...
# MAGIC %sql select * from loyalty_azlab_prod.headroom_allocation_p_tbl where campaign = 20230426 and cust_id = '6872732896086312431'

# COMMAND ----------

# MAGIC %sql select * from loyalty_azlab_prod.headroom_allocation_p_tbl where campaign = 20230426 and cust_id = '-7385606211536121860'
...
# MAGIC %md # investigate allocation 
...
if "allocate" in config.steps:
    ...
    headroom_export = (allocation_manager.get(predictions.filter(F.col("cust_id") == -3395896712043089052))
                       .withColumn("campaign", F.lit(campaign))
                       )
...
predictions.filter(F.col("cust_id") == -3395896712043089052).display()

# COMMAND ----------

prediction_sample = (predictions.filter(F.col("cust_id") == -3395896712043089052)).cache()
prediction_sample.display()
...
allocation_manager.get(prediction_sample).display()

# COMMAND ----------

prediction_scores = allocation_manager.get_prediction_scores(prediction_sample)
...
allocation_manager.max_increase, allocation_manager.min_increase
...
# MAGIC %md # investigate build dataset 
