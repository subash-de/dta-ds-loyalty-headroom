...
from cdsutils.io_utils import load_object
...
if "offline_eval" in config.steps:
    ...
    for seg in seg_list:
        seg_ext = [f"{k}={seg[k]}" for k in partitionByList]
        seg_pred_data_path = create_path_campaign(
            (config_ev["eval_data_path"] + seg_ext)
        )
        seg_data_processor_path = create_path_campaign(
            (config_ev["data_processor_path"] + seg_ext)
        )

        data_processor = load_object(seg_data_processor_path)
        data = (
            spark.read.parquet(seg_pred_data_path).select(
                user_key, pred_key, data_processor.feature_col
            )
        ).toPandas()
        ...
        for m in all_methods:
            ...
            algo_fn = partial(build_recommender, method=m)
            ...
            if config_ev["kfold"]:
                eval_summary = evaluator.evaluate_kfold(
                    data,
