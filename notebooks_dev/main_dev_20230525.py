    ...
    predictions = (
        predictions.withColumn(
            ...
            F.when(
                ...
                F.col("l2_id_total_spend_basket") * 1.05,
...
headroom_tbl = spark.read.parquet(
    "/mnt/centralds/offerallocation/headroom/analysis/230525/allocation_bought_into"
)
headroom_tbl.count()
...
headroom_subset.groupBy("desc").count().withColumn(
    "percentage", F.round(F.col("count") / F.sum("count").over(W.partitionBy()), 3)
).display()
...
headroom_subset.filter(
    F.col("desc") == "£3 off when you spend £20 on M&S food in store"
).orderBy(F.rand()).display()
...
headroom_subset.filter(
    F.col("desc") == "£3 off when you spend £20 on M&S food in store"
).orderBy(F.rand()).limit(30).display()

# COMMAND ----------

headroom_subset.filter(
    F.col("desc") == "£3 off when you spend £20 on M&S food in store"
).orderBy(F.rand()).limit(30).write.csv(
    "dbfs:/mnt/centralds/offerallocation/headroom/analysis/230525/sample_cust_3off20.csv"
)
