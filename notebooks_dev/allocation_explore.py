...
logger.info("Allocating all customers")
allocation_manager_l2 = Allocator(
    feature_col=config_al["feature_col"],
    offer_limits=config["offer_limits"],
    offer_desc=config["offers_desc"],
    user_key=config_al["user_key"],
...
headroom_export_l2 = (
    allocation_manager_l2.get(predictions).withColumn("campaign", F.lit(campaign))
).cache()
...
df = headroom_export_l2.groupBy("cust_id").agg(
    F.sum("spend_plus_headroom").alias("sum_spend_plus_headroom")
)
join_dfs = headroom_export.join(df, on="cust_id", how="inner").select(
    "cust_id", "spend_plus_headroom", "sum_spend_plus_headroom"
)
check = join_dfs.withColumn(
    "diff", F.col("sum_spend_plus_headroom") - F.col("spend_plus_headroom")
)
