...
if "allocate" in config.steps:
    ...
    if config_al["tcol_allocate_separately"]:
        ...
        headroom_export_top = allocation_manager.get(prediction_top).withColumn(
        ...
        headroom_export_not_top = allocation_manager.get(prediction_not_top).withColumn(
            "campaign", F.lit(campaign)
        )
...
headroom_export.groupBy("desc").count().withColumn(
    "percentage", F.round(F.col("count") / F.sum("count").over(W.partitionBy()), 3)
).display()
