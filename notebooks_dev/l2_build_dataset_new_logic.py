...
customer_lx_trans_grouped = (customer_lx_transactions
                            ...
                            # .groupby([self.user_key])
                              #                                      .pivot(f"{self.lx}_id")
                              .agg(F.count(f"{trx_manager.lx}_name")
...
df = (
    ...
    .withColumn(f"{trx_manager.lx}_id_total_time_window_spend", F.col('85percentile_total_spend_time_window'))
)
...
customer_lx_trans_grouped_all = (customer_lx_trans_grouped
                                  .join(customer_lx_time_window_spend, on = [trx_manager.user_key, f"{trx_manager.lx}_id"])
...
cust_lx_trx_metrics = cust_lx_trx_metrics.join(cust_seg
                                              .dropDuplicates(subset=[trx_manager.user_key]),
                                              on=trx_manager.user_key)
...
customer_transactions = (trx_line
                                 .select(trx_manager.user_key, "cust_age", "cust_gender", "article_id",
...
'''
lx_all = (lu_article
                  .filter(F.col('l1_id') == 'FD')
                  .select('article_id','l2_id','l2_name','l3_id','l3_name')
