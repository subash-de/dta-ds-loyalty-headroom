...
# partitionByList = config["fit_rec"]["partitionByList"]
# "_".join([str(seg_list[11][k]) for k in partitionByList] )
...
predictions.groupby("l2_id").agg(F.count("*").alias("count")).groupby("count").count().display()
...
p1 = allocation_manager.get_prediction_scores(predictions.filter(F.col("cust_id") == 6872732896086312431))
display(p1)

# COMMAND ----------

p2 = allocation_manager.tag_outliers(p1)
display(p2)

# COMMAND ----------

display(predictions.filter(F.col("cust_id") == 6872732896086312431)
...
# MAGIC %sql select distinct l2_id from loyalty_azlab_prod.predictions_230522_p_tbl group by cust_id, l2_id

# COMMAND ----------

# MAGIC %sql select distinct campaign from loyalty_azlab_prod.predictions_230522_p_tbl
...
pred_2 = spark.sql("select * from loyalty_azlab_prod.predictions_230522_p_tbl")

# COMMAND ----------

pred_2.groupby("cust_id").agg(F.count("*").alias("count_row")).filter(F.col("count_row") != 7 ).display()
