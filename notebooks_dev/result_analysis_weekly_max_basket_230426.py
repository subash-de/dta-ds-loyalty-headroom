...
alloc = spark.read.parquet(
    "/mnt/centralds/offerallocation/headroom/analysis/230426/allocation"
)
alloc.count()
...
alloc_combined.groupBy("desc").count().withColumn(
...
alloc_spend_gt_x.groupby("desc").count().display()
...
alloc_spend_gt_x.groupby("segment_desc").agg(
    F.sum("spend_needed_to_redeem"),
    F.sum("spend_needed_ex_headroom"),
    F.sum("markdown"),
).display()
